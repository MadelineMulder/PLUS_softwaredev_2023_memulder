...
# The goal of this notebook is to use the folium library to create a bubble web map showing hurricane intensity (using fake hurricane data for the past few decades) for Grand Bahama Island. Our group's final project is related to emergency routing in the face of flood hazards caused by hurricanes in the Bahamas, so this notebook aims to explore one way in which to visualize that. 
# 
# This notebook uses the folium library (https://python-visualization.github.io/folium/). More specifically, it uses the .Map function (https://python-visualization.github.io/folium/modules.html#module-folium.map), the .GeoJSON function (under the extra features sections of the documentation), and the .CircleMarker and the .Marker functions (under the vector layers section of the documentation) . 
# 
# Additionally, this notebook uses pandas, specifically the .Dataframe function (https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html?highlight=dataframe#pandas.DataFrame), for the creation and management of data, and os, specifically .path.isdir and .path.join (https://docs.python.org/3/library/os.path.html), to create a correct file path in order to save the web map.
# 
# The bubble map was adapted from: https://nagasudhir.blogspot.com/2021/08/create-bubble-map-from-excel-data-using.html https://www.python-graph-gallery.com/313-bubble-map-with-folium?utm_content=cmp-true#google_vignette
